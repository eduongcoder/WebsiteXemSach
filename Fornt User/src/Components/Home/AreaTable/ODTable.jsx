...
function OrderTable() {
    ...
    return (
        ...
        <div className="mb-3">
            <h4 className="text-[18px] text-sky-400">Latest Orders</h4>
        </div>
                ...
                <tbody>
                    {TABLE_DATA.map((dataItem) => (
                        <tr key={dataItem.id}>
                            ...
                            <td className="px-3 py-3">{dataItem.order_id}</td>
                            <td className="px-3 py-3">{dataItem.date}</td>
                            <td className="px-3 py-3">{dataItem.customer}</td>
                            <td className="px-3 py-3">
                                <div className="flex items-center gap-2">
                                    <span className={`w-2 h-2 rounded-full dot-${dataItem.status}`}></span>
                                    <span className="capitalize">{dataItem.status}</span>
                                </div>
                            </td>
                            <td className="px-3 py-3">
                                ${typeof dataItem.amount === 'number' ? dataItem.amount.toFixed(2) : dataItem.amount}
                            </td>
...
export default OrderTable; // Đổi tên hàm cho phù hợp với tên hàm đã sửa
