...
export const FetchCommentNovel = createAsyncThunk(
    ...
    async (idNovel, { rejectWithValue }) => {
        try {
            ...
            console.log('dfgyhuijoklmjg', response.data);
            return response.data.result;
        } catch (error) {
            return rejectWithValue(
                error.response?.data?.message ||
                    'Failed to fetch comment novel',
            );
        }
    },
);
export const LikeComment = createAsyncThunk(
    ...
    async (idComment, { rejectWithValue }) => {
        try {
            await axios.put(`${BASE_URL}/likeComment?idComment=${idComment}`);
            return idComment;
        } catch (error) {
            return rejectWithValue(
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(LikeComment.fulfilled, (state, action) => {
                ...
                const array = JSON.parse(JSON.stringify(state.commentsNovels));
                console.log(array);
                state.commentsNovels = array.map((el) =>
                    el.idComment == result
                        ? { ...el, likeComment: el.likeComment + 1 }
                        : el,
                );
            })
            .addCase(DisLikeComment.fulfilled, (state, action) => {
                ...
                console.log(array);
                state.commentsNovels = array.map((el) =>
                    el.idComment == action.payload
                        ? { ...el, dislikeComment: el.dislikeComment + 1 }
                        : el,
                );
            })
